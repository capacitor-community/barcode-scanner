...
public class BarcodeScanner: CAPPlugin, AVCaptureMetadataOutputObjectsDelegate {
    ...
    private func scan() {
        if (!self.didRunCameraPrepare) {
            ...
            DispatchQueue.main.async {
                let scanViewOfInterest = self.savedCall?.getObject("scanViewOfInterest")
                ...
                if let rect = self.captureVideoPreviewLayer?.rectOfInterestConverted(
                    ...
                    fromLayerRect: CGRect(x: scanViewOfInterest.x ?? 0, y: scanViewOfInterest.y ?? 0, width: scanViewOfInterest.width ?? UIScreen.main.bounds.width, height: scanViewOfInterest.height ?? UIScreen.main.bounds.height)
                ) {
